...
async def login(request: Request):
    """Password login for the hosted web app. Returns a session token on
    success. Only meaningful when HOSTED_MODE is on and AUTH_PASSWORD is set."""
...
async def auth_config():
    """Lets the frontend know whether it needs to show a login screen at all."""
    return JSONResponse({
        "hosted_mode": ada.HOSTED_MODE,
        "auth_required": ada.HOSTED_MODE and auth.is_auth_configured(),
    })
...
# Buffers the current in-progress utterance per speaker, so we save one
# complete message to persistent memory per turn rather than every tiny
# transcription delta.
_memory_turn_buffer = {"sender": None, "text": ""}

def _flush_memory_buffer():
    global _memory_turn_buffer
    sender = _memory_turn_buffer["sender"]
    text = _memory_turn_buffer["text"]
    if sender and text.strip():
        asyncio.create_task(asyncio.to_thread(memory_store.save_entry, sender, text))
    _memory_turn_buffer = {"sender": None, "text": ""}

async def _auto_load_memory():
    """Pulls recent conversation history from persistent storage (Supabase)
    and injects it into the just-started session, so Jarvis remembers past
    conversations even from other devices. No-ops quietly if memory_store
    isn't configured, or if the session doesn't come up in time."""
    if not memory_store.is_configured():
        return
    for _ in range(20):  # wait up to ~10s for the live session to be ready
        if audio_loop and audio_loop.session:
            break
        await asyncio.sleep(0.5)
    if not (audio_loop and audio_loop.session):
        print("[memory_store] Session not ready in time, skipping auto-load.")
        return
    memory_text = await asyncio.to_thread(memory_store.get_recent_as_text, 200)
    if not memory_text:
        return
    context_msg = (
        "System Notification: Here is a log of your previous conversations "
        "with this user from earlier sessions (possibly on a different "
        "device). Use it for continuity, but respond naturally - don't "
        "recite it back verbatim:\n\n" + memory_text
    )
    try:
        await audio_loop.session.send(input=context_msg, end_of_turn=True)
        print("[memory_store] Auto-loaded previous memory into session.")
        await sio.emit('status', {'msg': 'Loaded memory from previous sessions'})
    except Exception as e:
        print(f"[memory_store] Failed to send memory context: {e}")
authenticator = None
...
async def connect(sid, environ, auth_data=None):
    # When hosted publicly with a password configured, require a valid
    # session token before allowing the socket connection at all.
    if ada.HOSTED_MODE and auth.is_auth_configured():
        token = (auth_data or {}).get('token') if auth_data else None
        if not auth.verify_token(token):
            print(f"Rejected unauthenticated connection: {sid}")
            raise socketio.exceptions.ConnectionRefusedError('Authentication required')

    print(f"Client connected: {sid}")
    await sio.emit('status', {'msg': 'Connected to A.D.A Backend'}, room=sid)
    # Tell the frontend whether this backend has real local hardware (mic/
    # camera/speakers) or is running on a cloud server, so it knows whether
    # to capture mic audio and play AI audio itself via the browser.
    await sio.emit('hosted_mode', {'hosted': ada.HOSTED_MODE}, room=sid)
...
async def start_audio(sid, data=None):
    ...
    def on_transcription(data):
        ...
        # Also buffer this into persistent cross-device memory. Deltas for
        # the same speaker accumulate; a speaker change flushes the previous
        # speaker's completed utterance to storage.
        sender = data.get("sender")
        text = data.get("text", "")
        if _memory_turn_buffer["sender"] not in (None, sender):
            _flush_memory_buffer()
        _memory_turn_buffer["sender"] = sender
        _memory_turn_buffer["text"] += text
    ...
    try:
        ...
        # Automatically pull in memory from previous sessions (possibly on a
        # different device) and load it into this session's context, so
        # Jarvis feels continuous across devices instead of starting fresh
        # every time.
        asyncio.create_task(_auto_load_memory())
...
async def mic_audio_chunk(sid, data):
    """Receives raw 16-bit PCM, 16kHz mono audio captured by the browser's
    microphone (used when running in HOSTED_MODE, i.e. cloud deployment,
    where there's no local mic for the backend to read from directly).
    data: { audio: base64-encoded PCM bytes }"""
    if not audio_loop:
        return
    ...
    try:
        ...
        return
    asyncio.create_task(audio_loop.send_audio_chunk(pcm_bytes))
...
async def save_memory(sid, data):
    try:
        ...
        # Write to file (local backup copy - not the primary memory store,
        # see memory_store.py for the persistent, cross-device version)
        with open(filename, 'w', encoding='utf-8') as f:
            for msg in messages:
                sender = msg.get('sender', 'Unknown')
                text = msg.get('text', '')
                f.write(f"{sender}: {text}\n")
                # Also persist each message into the cross-device memory store
                asyncio.create_task(asyncio.to_thread(memory_store.save_entry, sender, text))
...
@sio.event
async def spotify_status(sid):
    """Report whether Spotify is configured and what's currently playing."""
    if not spotify_agent.is_configured():
        await sio.emit('spotify_status', {
            'configured': False,
            'msg': "Spotify not configured. Add SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET to .env"
        })
        return
    try:
        current = await asyncio.to_thread(spotify_agent.get_current)
        await sio.emit('spotify_status', {'configured': True, 'current': current})
    except Exception as e:
        await sio.emit('spotify_status', {'configured': True, 'current': None, 'error': str(e)})

@sio.event
async def spotify_play(sid, data=None):
    """data: { query: 'song name' } to search+play, or {} to resume playback."""
    data = data or {}
    query = data.get('query')
    try:
        if query:
            # Search-and-launch: works on Free accounts, no login needed.
            # Opens the track link, which launches the local Spotify app.
            track = await asyncio.to_thread(spotify_agent.launch_search, query)
            if track:
                await sio.emit('status', {'msg': f"Opening '{track['name']}' by {track['artist']} on Spotify"})
                await sio.emit('spotify_update', {'action': 'play', 'track': track})
            else:
                await sio.emit('error', {'msg': f"No results found for '{query}'"})
        else:
            # Resuming playback on a specific device requires Premium + OAuth.
            await asyncio.to_thread(spotify_agent.play)
            await sio.emit('status', {'msg': "Resumed Spotify playback"})
            await sio.emit('spotify_update', {'action': 'play'})
    except Exception as e:
        msg = str(e)
        if "403" in msg or "PREMIUM_REQUIRED" in msg.upper():
            msg = "Spotify playback control requires a Premium account."
        await sio.emit('error', {'msg': f"Spotify Play Failed: {msg}"})

@sio.event
async def spotify_pause(sid, data=None):
    try:
        await asyncio.to_thread(spotify_agent.pause)
        await sio.emit('status', {'msg': "Paused Spotify"})
        await sio.emit('spotify_update', {'action': 'pause'})
    except Exception as e:
        await sio.emit('error', {'msg': f"Spotify Pause Failed: {str(e)}"})

@sio.event
async def spotify_next(sid, data=None):
    try:
        await asyncio.to_thread(spotify_agent.next_track)
        await sio.emit('status', {'msg': "Skipped to next track"})
        await sio.emit('spotify_update', {'action': 'next'})
    except Exception as e:
        await sio.emit('error', {'msg': f"Spotify Skip Failed: {str(e)}"})

@sio.event
async def spotify_previous(sid, data=None):
    try:
        await asyncio.to_thread(spotify_agent.previous_track)
        await sio.emit('status', {'msg': "Went back a track"})
        await sio.emit('spotify_update', {'action': 'previous'})
    except Exception as e:
        await sio.emit('error', {'msg': f"Spotify Previous Failed: {str(e)}"})

@sio.event
async def spotify_volume(sid, data):
    percent = data.get('value', 50)
    try:
        await asyncio.to_thread(spotify_agent.set_volume, percent)
        await sio.emit('status', {'msg': f"Volume set to {percent}%"})
        await sio.emit('spotify_update', {'action': 'volume', 'value': percent})
    except Exception as e:
        await sio.emit('error', {'msg': f"Spotify Volume Failed: {str(e)}"})
...
@sio.event
async def get_time(sid, data=None):
    result = basic_agent.get_time()
    await sio.emit('status', {'msg': f"It's {result['time']}"})
    await sio.emit('time_result', result)

@sio.event
async def get_date(sid, data=None):
    result = basic_agent.get_date()
    await sio.emit('status', {'msg': f"Today is {result['date']}"})
    await sio.emit('date_result', result)

@sio.event
async def get_weather(sid, data):
    location = (data or {}).get('location', '')
    if not location:
        await sio.emit('error', {'msg': "Please specify a location for the weather."})
        return
    try:
        result = await asyncio.to_thread(basic_agent.get_weather, location)
        await sio.emit('status', {
            'msg': f"{result['location']}: {result['temperature_f']}°F, "
                   f"high {result['high_f']}° / low {result['low_f']}°"
        })
        await sio.emit('weather_result', result)
    except Exception as e:
        await sio.emit('error', {'msg': f"Weather lookup failed: {str(e)}"})

@sio.event
async def take_screenshot(sid, data=None):
    try:
        result = await asyncio.to_thread(basic_agent.take_screenshot)
        await sio.emit('status', {'msg': f"Screenshot saved to {result['path']}"})
        await sio.emit('screenshot_result', result)
    except Exception as e:
        await sio.emit('error', {'msg': f"Screenshot failed: {str(e)}"})

@sio.event
async def get_system_stats(sid, data=None):
    try:
        result = await asyncio.to_thread(basic_agent.get_system_stats)
        await sio.emit('system_stats_result', result)
    except Exception as e:
        await sio.emit('error', {'msg': f"System stats failed: {str(e)}"})

@sio.event
async def open_url(sid, data):
    query = (data or {}).get('query', '')
    if not query:
        await sio.emit('error', {'msg': "Please specify what to open."})
        return
    try:
        result = await asyncio.to_thread(basic_agent.open_url, query)
        await sio.emit('status', {'msg': f"Opening {result['opened']}"})
    except Exception as e:
        await sio.emit('error', {'msg': f"Open failed: {str(e)}"})
...
# Serve the built frontend (npm run build output) if present, so this one
# backend can serve both the API/socket AND the website - simplest possible
# deployment (one Render service, one URL). In local dev, this directory
# won't exist (you use `npm run dev` / Vite instead), so this quietly skips.
_frontend_dist = Path(__file__).parent.parent / "dist"
if _frontend_dist.exists():
    app.mount("/", StaticFiles(directory=str(_frontend_dist), html=True), name="frontend")
    print(f"[SERVER] Serving built frontend from {_frontend_dist}")
else:
    print("[SERVER] No built frontend found (dist/) - running in API-only mode. "
          "This is normal for local dev with `npm run dev`.")

if __name__ == "__main__":
    # Render (and most cloud hosts) assign a port dynamically via $PORT and
    # require binding to 0.0.0.0, not 127.0.0.1. Locally, this still
    # defaults to 127.0.0.1:8000 exactly as before.
