...
# When running on a cloud server (no physical mic/camera/speakers attached),
# set HOSTED_MODE=true in the environment. This disables all server-side
# hardware access. Audio/video instead flow entirely over the socket
# connection to/from the browser, which has the real mic/camera/speakers.
...
config = types.LiveConnectConfig(
    ...
    system_instruction="Your name is Jarvis, a highly capable personal AI assistant. "
        "You have a witty and charming personality. "
        "You address your user as 'Sir'. "
...
class AudioLoop:
    ...
    async def send_audio_chunk(self, pcm_bytes):
        """Feed a raw 16-bit PCM, 16kHz mono audio chunk (captured by the
        browser's microphone) into the Gemini session. Used in HOSTED_MODE
        instead of listen_audio(), which reads from local hardware directly.
        Mirrors listen_audio()'s VAD logic so speech-triggered video frames
        still work."""
        if self.paused or not self.out_queue:
            return
    ...
    async def listen_audio(self):
        if HOSTED_MODE:
            # In hosted mode there's no local microphone to read from - audio
            # arrives from the browser via send_audio_chunk() instead (wired
            # up to a socket event in server.py). Just idle here.
            while True:
    ...
    async def play_audio(self):
        stream = None
        if not HOSTED_MODE:
            # Local desktop mode: also play out of this machine's speakers.
    ...
    async def get_frames(self):
        if HOSTED_MODE:
            # Video frames arrive via the browser's `video_frame` socket event
            # (already wired up for hand-tracking) instead of a local camera.
            while True:
                await asyncio.sleep(3600)
