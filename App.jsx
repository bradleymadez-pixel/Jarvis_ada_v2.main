...
import JarvisWindow from './components/JarvisWindow';
import PasswordLogin from './components/PasswordLogin';
import useJarvisAudio from './hooks/useJarvisAudio';



// In local dev (npm run dev / Electron), the backend runs on localhost:8000.
// In production (hosted web app), set VITE_BACKEND_URL at build time to your
// deployed backend's URL, e.g. https://jarvis-backend.onrender.com
// If unset in production, defaults to same-origin (works if frontend and
// backend are served from the same domain).
const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || (import.meta.env.DEV ? 'http://localhost:8000' : undefined);
const STORED_TOKEN_KEY = 'jarvis_auth_token';
const socket = io(BACKEND_URL, {
    autoConnect: false,
    auth: { token: localStorage.getItem(STORED_TOKEN_KEY) || null },
});
// In a browser (web app mode), window.require doesn't exist - only present
// inside Electron's renderer process. Some browser extensions define a
// window.require that isn't actually Electron's, so we check it's really a
// function AND wrap the call in try/catch as a second safety net - either
// way, fall back to no-ops so the app runs fine as a plain website too.
const ipcRenderer = (() => {
    ...
    try {
        ...
        console.warn('Not running inside Electron - window controls disabled.', err);
        return { send: () => {} };
    }
})();
...
function App() {
    ...
    const [needsPasswordLogin, setNeedsPasswordLogin] = useState(false);
    const [authCheckDone, setAuthCheckDone] = useState(false);

    // Browser-side mic capture + AI voice playback - only actually does
    // anything when the backend reports HOSTED_MODE (a cloud server with no
    // local mic/camera/speakers of its own).
    useJarvisAudio({ socket, isHostedMode, isMuted, isConnected });

    // Before doing anything else, check whether the backend requires a
    // password login (only true for a public hosted deployment with
    // AUTH_PASSWORD set). Local/Electron use skips this entirely.
    useEffect(() => {
        const checkAuth = async () => {
            try {
                const base = BACKEND_URL || window.location.origin;
                const res = await fetch(`${base}/api/auth_config`);
                const data = await res.json();
                if (data.auth_required) {
                    const existingToken = localStorage.getItem(STORED_TOKEN_KEY);
                    if (existingToken) {
                        // Try connecting with the stored token; if it's stale,
                        // connect_error below will kick us back to the login screen.
                        socket.connect();
                    } else {
                        setNeedsPasswordLogin(true);
                    }
                } else {
                    socket.connect();
                }
            } catch (err) {
                console.error('Could not reach backend for auth check:', err);
                // Fall back to trying a direct connection anyway.
                socket.connect();
            } finally {
    ...
    const handlePasswordAuthenticated = (token) => {
        localStorage.setItem(STORED_TOKEN_KEY, token);
        socket.auth = { token };
        setNeedsPasswordLogin(false);
        socket.connect();
    };
    ...
    const [showJarvisWindow, setShowJarvisWindow] = useState(false);
    ...
    const [elementPositions, setElementPositions] = useState({
        ...
        'jarvis-basic': { x: window.innerWidth / 2, y: window.innerHeight / 2 - 250 },
    ...
    useEffect(() => {
        ...
        socket.on('hosted_mode', (data) => {
            setIsHostedMode(!!data.hosted);
        });
    ...
    const toggleJarvisWindow = () => {
        setShowJarvisWindow(!showJarvisWindow);
    };
    ...
    return (
        <div className="h-screen w-screen bg-black text-cyan-100 font-mono overflow-hidden flex flex-col relative selection:bg-cyan-900 selection:text-white">
            ...
            {/* Password login for hosted web-app deployments (public internet access).
                Takes priority over everything else - nothing else renders meaningfully
                until this is passed, since the socket won't even connect yet. */}
            {needsPasswordLogin && (
                <PasswordLogin
                    backendUrl={BACKEND_URL || window.location.origin}
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
                ...
                <div className="z-20 flex justify-center pb-10 pointer-events-none">
                    <ToolsModule
                        ...
                        onToggleJarvis={toggleJarvisWindow}
                        showJarvisWindow={showJarvisWindow}
                        activeDragElement={activeDragElement}
                ...
                {/* Jarvis Basics Window (no-AI commands: time, weather, spotify, screenshot, stats) */}
                {showJarvisWindow && (
                    <JarvisWindow
                        socket={socket}
                        position={elementPositions['jarvis-basic']}
                        onClose={() => setShowJarvisWindow(false)}
                        onMouseDown={(e) => handleMouseDown(e, 'jarvis-basic')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        zIndex={getZIndex('jarvis-basic')}
                    />
                )}
